...
## Key Files
- `reader.html` — Walkthrough reader app. Imports `marked.js` for markdown rendering, fetches `guides.json` to choose a guide, loads `<guide-path>/toc.json` for sidebar TOC and `<guide-path>/meta.json` for title/author, loads per-section `.md` files on navigation.
...
- `guides/<slug>/` — Walkthrough section files per game: `index.md` (Table of Contents), `toc.json` (machine-readable TOC), `meta.json` (title/author/attribution), and one `.md` file per section.
...
## Conventions
- `<guide-path>/index.md` must link to all section files with correct filenames
...
- `<guide-path>/meta.json` must contain `title`, `subtitle`, `author`, and `attributionHtml`
...
- `guides/<slug>/` section files are generated by the `faqmd` converter — do not edit
  them by hand.
- To add a new walkthrough, build it in the `faqmd` repo, copy the generated
  `guide/` directory (from the converter) to `gamemds/guides/<slug>/`, add an entry
  to `guides.json`, and push to `main`.
...
1. In the **[faqmd](https://github.com/danielcurran/faqmd)** repo:
   ...
   - Test: `npm test`
2. Copy the generated `guide/` directory (from the faqmd converter) to `gamemds/guides/<slug>/`.
