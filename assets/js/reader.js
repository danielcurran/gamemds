...
function loadEinkMode() {
  try { return localStorage.getItem(EINK_STORAGE_KEY) === '1'; } catch { return false; }
}

function saveEinkMode(on) {
  try { localStorage.setItem(EINK_STORAGE_KEY, on ? '1' : '0'); } catch {}
}

function applyEinkMode() {
  einkMode = loadEinkMode();
  const btn = $('eink-btn');
  if (einkMode) {
